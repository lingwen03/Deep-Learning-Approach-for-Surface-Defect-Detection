"""
Optimized inference script v3 — bilateral filter + CLAHE + letterbox.

Key improvements over inference_v2.py:
  1. Bilateral filter: removes highlight speckles while preserving defect edges
     (Pain Point 1 fix — eliminates false positives from lighting artifacts)
  2. CLAHE preprocessing: enhances local contrast for subtle defects
  3. Letterbox resize: preserves aspect ratio (no geometric distortion)
  4. Mask post-processing: morphological cleaning + connected component filter
  5. Overlapping-crop ensemble for large images with cosine-weighted stitching

Usage: python inference_v3.py --image_dir <path_to_images>
"""
import argparse
import os
import sys
...
def apply_bilateral(img, d=9, sigma_color=75, sigma_space=75):
    """
    Pain Point 1 fix: bilateral filter.

    Smooths background noise and highlight speckles from industrial lighting
    while preserving defect boundary edges (scratches, burrs, pits).

    d=9:        filter diameter — large enough for highlight suppression
    sigmaColor=75:  strong color-domain smoothing
    sigmaSpace=75:  strong spatial-domain smoothing
    """
    return cv2.bilateralFilter(img, d, sigma_color, sigma_space)


def apply_clahe(img, clip_limit=2.0, tile_size=8):
    """Apply CLAHE to enhance local contrast for subtle defects."""
    clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=(tile_size, tile_size))
    return clahe.apply(img)


def letterbox_resize(img, target_h=TARGET_H, target_w=TARGET_W):
    """Resize to target while preserving aspect ratio; pad with border replication."""
    h, w = img.shape
...
def preprocess_pipeline(img, bilateral=True, clahe=True):
    """
    Full preprocessing pipeline:
      raw -> bilateral (denoise + de-highlight) -> CLAHE (enhance) -> ready

    This mirrors the training-time preprocessing in data_manager.py.
    """
    if bilateral:
...
def postprocess_mask(mask_raw, min_area_ratio=0.0001, morph_close_size=7, morph_open_size=5):
    """Clean raw mask: Otsu threshold -> close (fill gaps) -> open (remove noise) -> CC filter."""
    h, w = mask_raw.shape
    total_pixels = h * w
...
def ensemble_crop_inference(sess, full_img, mask_tensor, image_ph, crop_overlap=True):
    ...
    if h <= TARGET_H * 1.3 and w <= TARGET_W * 1.3:
        img_enhanced = preprocess_pipeline(full_img)
    ...
    overlap_h = TARGET_H // 4
    overlap_w = TARGET_W // 4
    ...
    for crop_idx, (top, left, bottom, right) in enumerate(crops):
        ...
        crop_enhanced = preprocess_pipeline(crop)
...
def run_inference_v3(image_dir, checkpoint_dir, output_dir,
                     use_bilateral=True, use_clahe=True, use_crop_ensemble=True,
                     min_area_ratio=0.0001, morph_close=7, morph_open=5):
    """Run optimized defect detection with bilateral + CLAHE + letterbox pipeline."""
    ...
    print("Config: Bilateral={}, CLAHE={}, crop_ensemble={}, morph_close={}, morph_open={}, min_area_ratio={}".format(
        use_bilateral, use_clahe, use_crop_ensemble, morph_close, morph_open, min_area_ratio))

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # Build model graph (same architecture, unchanged)
    ...
    with tf.Session() as sess:
        ...
        for idx, image_file in enumerate(image_files):
            ...
            # Step 0.5: Apply bilateral filter (Pain Point 1)
            # This happens BEFORE CLAHE so they compose: bilateral de-noises
            # lighting speckles, then CLAHE amplifies the remaining signal
            if use_bilateral:
                img_clean = apply_bilateral(original_img)
            else:
                img_clean = original_img

            # Step 1: Get raw mask
            img_enhanced = apply_clahe(img_clean) if use_clahe else img_clean

            if use_crop_ensemble and (oh > TARGET_H * 1.3 or ow > TARGET_W * 1.3):
                # Ensemble path: pass clean (not enhanced) to the ensemble function
                # which applies the full preprocessing pipeline internally
                ...
                mask_raw = (np.array(mask_out[0]).squeeze(2) * 255).astype(np.uint8)
                mask_letterbox = cv2.resize(mask_raw, (TARGET_W, TARGET_H),
                                            interpolation=cv2.INTER_LINEAR)
                mask_raw = remove_padding(mask_letterbox, pad_info)
            ...
            for j, r in enumerate(defect_regions[:5]):
            ...
            # 4a. Three-panel comparison: original | bilateral-cleaned | final mask
            # Build a 3-panel: [original] [bilateral+CLAHE enhanced] [cleaned mask]
            comparison = concatImage([original_img, img_enhanced, mask_cleaned])
            comp_path = os.path.join(output_dir, "{}_comparison.png".format(basename))
            comparison.save(comp_path)

            # 4b. Overlay: original + red defect regions
            overlay = cv2.cvtColor(original_img, cv2.COLOR_GRAY2BGR)
            mask_colored = np.zeros((oh, ow, 3), dtype=np.uint8)
            mask_colored[:, :, 2] = mask_cleaned
            ...
            # 4d. Bilateral+CLAHE enhanced reference
        ...
        # Summary
        print("\n" + "=" * 60)
        print("SUMMARY  (v3: bilateral + CLAHE + letterbox)")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description='Defect detection inference v3 (bilateral + CLAHE)')
    parser.add_argument('--image_dir', type=str, required=True)
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoint')
    parser.add_argument('--output_dir', type=str, default='inference_results_v3')
